...
int main(int argc, char* argv[]) {
  ...
  graybg = cvCloneImage(gray);
  diffImg = cvCloneImage(graybg);
  cvCvtColor(src, graybg, CV_BGR2GRAY);
  ...
  while (1) {
    ...
    cvThreshold(diffImg, bin, 18, 255, 0);
    ...
    if(key == 'c'){
      cvSaveImage("result.jpg", rendered, 0);
    }
...
void process(IplImage* input, IplImage* destination) {
  CvSize size;
  int step;
  uchar* data;
  uchar* labeldata;
