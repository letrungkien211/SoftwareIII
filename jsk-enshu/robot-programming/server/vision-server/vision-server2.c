/*****************************************
  重心だけでなく面積も返すサーバ
   ...
   vision-server.cと比較して、
    関数 calcCentroidで画像面積areaを計算している所、
    関数 interpreterの中の、  
       else if(strcmp(com, "result") == 0){
     の部分で、:centroidだけでなく
     :areaをclientに渡す結果として与えていることで機能拡張している

  また、clientからの閾値指定がきた場合のコマンド
   color-threshold #f(rmin rmax gmin gmax bmin bmax)
  がきた場合に、閾値の処理を変えることができるように変更
  また、これに伴い、
  robot-client.lをrobot-client2.lとすることで、コマンドを追加
...
int interpreter(char *line){
  ...
  //  ここでは重心位置を返すようにしている
    ...
    size = sprintf(result, "((:centroid #f(%d %d)) (:area %d))\n", 
...
int main (int argc, char **argv){
  ...
  while(1){
    ...
    centroid = calcCentroid(processed_image, grayImage, binaryImage); //重心,面積計算
    drawCross(processed_image, &centroid, CV_RGB(0, 255, 0)); //重心位置に×
    sprintf(text, "X: %d Y: %d AREA: %d", centroid.x, centroid.y, area);     //文字書き込み
