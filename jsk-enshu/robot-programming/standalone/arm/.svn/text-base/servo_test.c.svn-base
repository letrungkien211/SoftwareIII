...
int command_poweronflag(int argc, char *argv[]);
...
struct command command_list[] =
  {{"axis", &command_axis},
   ...
   {"poweronflag", &command_poweronflag}
  };
...
int command_axis(int argc, char *argv[]) {
  int tmp_axis;
  if (argc > 1) sscanf(argv[1], "%d", &tmp_axis);
  if(tmp_axis != axis){
    servos[axis].poweronflag = 0;
  }
  axis = tmp_axis;
  if (axis < 0) axis = 0;
  else if(axis >= SERVO_NUM) axis = SERVO_NUM - 1;
  printf("axis = %d\n", axis);
  servos[axis].poweronflag = 1;
  return TRUE;
}
/* set servo pulse */
int command_pulse(int argc, char *argv[]) {
...
/* set servo poweronflag */
int command_poweronflag(int argc, char *argv[]) {
  int flag;
  int ch;
  if (argc > 1) {
    for (ch = 0; ch < (argc - 1) && ch < SERVO_NUM; ch ++) {
      sscanf(argv[ch+1], "%d", &flag);
      if(flag > 0)
        flag = 1;
      else
        flag = 0;
      servos[ch].poweronflag = flag;
    }
  }
  else {
    for (ch = 0; ch < SERVO_NUM; ch ++) printf("%d ", servos[ch].poweronflag);
    printf("\n");
  }
  return TRUE;
}
