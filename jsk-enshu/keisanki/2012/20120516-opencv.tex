...
\title{2012年度 機械工学総合演習\\計算機演習9 応用(実世界画像処理)}
...
実世界の情報を計算機の中に取り込み処理を行う実世界情報処理の一つとして，
カメラデバイスを用いた画像処理アプリケーションに関する実習を行う．
具体的には，カメラデバイスと画像処理ライブラリOpenCVを用いた計算機プロ
グラミング演習を行い，ライブラリの使い方，画像ファイル，カメラデバイス
の扱い方を習得する．
...
ホームページは\url{http://code.opencv.org/projects/opencv/wiki}にあり，
\url{http://pr.willowgarage.com/wiki/OpenCVMeetingNotes}で日々の開発状
況を知ることができる．
また問題点や修正情報，あるいは機能要求は
http://code.opencv.org/projects/opencv/issuesからバグトラッキングシス
テムを通じて報告することができ，だれでも開発に参加することができる．
さらに，ドキュメントは\url{http://docs.opencv.org/}を参照しよう．

OpenCVにはバージョン１とバージョン2が存在し，その間では大きくAPIが変更，
更新されているが，現在はバージョン2が主流になっている．演習計算機にイ
ンストールされている物もバージョン２である．日本語の情報はバージョン１
に基づく物も多いので注意すること．google.co.jpのトップページから右下の
Google.com in Englishのリンクをたどると英語ページだけを対象にした検索
が可能になる．
...
http://sourceforge.net/projects/jsk-enshu/files/keisanki-opencv-2012.tgz/download
に今回の演習に必要なファイルがある．
\begin{verbatim}
$ wget http://sourceforge.net/projects/jsk-enshu/files/keisanki-opencv-2012.tgz/download
 -O keisanki-opencv-2012.tgz
\end{verbatim}
としてコマンドラインからダウンロードすることができる．
...
このファイルは
https://code.ros.org/gf/project/opencv/scmsvn/?action=browse\&path= \newline
\%2F\%2Acheckout\%2A\%2Ftrunk\%2Fopencv\%2Fsamples\%2Fc\%2Flena.jpg
