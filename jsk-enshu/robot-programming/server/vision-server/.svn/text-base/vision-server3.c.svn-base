/*****************************************
  Labelingして複数結果を返すようにしたもの
   resultでの書式は、
   ((:centroid #f(x0 y0) #f(x1 y1) ...) (:area a0 a1 ..))
   のように、面積が大きいものから順に、座標、面積がリストで渡される

   vision-server.cと比較して、
    画像処理部分にlabeling関数が書かれていること
    関数 interpreterの中の、  
       else if(strcmp(com, "result") == 0){
     の部分で、複数のcentroid,areaを渡すようにして機能拡張している
...
#define PROCESS_WIDTH  320    //処理画像データの横
#define PROCESS_HEIGHT 240    //処理画像データの縦
